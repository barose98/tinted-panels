...
<xsl:variable name="sizesdoc" select="document('../panel-sizes.xml')"/>
...
panel_size =<xsl:value-of select="$sizesdoc/sizes/panel[@name=$panelname]/@size"/>
panel_margin = 0 0
panel_padding = 2 0 2
...
#vpos hpos orient
panel_position = <xsl:value-of select="$sizesdoc/sizes/panel[@name=$panelname]/@vside"/><xsl:text> </xsl:text> <xsl:value-of select="$sizesdoc/sizes/panel[@name=$panelname]/@hside"/><xsl:text> </xsl:text> <xsl:value-of select="$sizesdoc/sizes/panel[@name=$panelname]/@orient"/>
panel_layer = top
panel_monitor = all
panel_shrink = 1
autohide = 0
...
launcher_tooltip = 1
launcher_apps_dir = ~/Projects/DEV/BASH/panel/apps/launchers/tint
</xsl:template>
...
clock_lclick_command = qterminal  --geometry=70x38  -e &quot;cal -y &amp;&amp; read -n 1 c&quot;
...
border_color =<xsl:value-of select="$colors/colors/color[@name = $border]"/> 100
